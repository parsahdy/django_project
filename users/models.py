...
class UserManager(BaseUserManager):
    ...
    def create_user(self, username=None, phone_number=None, email=None, password=None, **extra_fields):
        if username is None:
            ...
            if phone_number:
                username = random.choice('abcdefghijklomnpqrstuvwxyz') + str(phone_number)[-7:]
...
class User(AbstractBaseUser, PermissionsMixin):
    """
    An abstract base class implenting a fully features User model with admin-compliant permissions.

    Username, password and email are required. other fields are optional.
    """
    username = models.CharField(_('username'), max_length=32, unique=True,
                                help_text=_(
                                    'Required, 30 characters or fewer starting with a letter, letters, digits and symbols.'),
    ...
    phone_number = models.CharField(_('mobile number'), max_length=15, unique=True, null=True, blank=True)
                                        #  validators=[
    ...
    is_active = models.BooleanField(_('sctive'), default=True,
                                    help_text=_('Designates whether this user should treated as active.'
                                                'Unselected this instead of deleting accounts.'))
    
    date_joind = models.DateTimeField(_('date joind'), default=timezone.now)
...
class UserProfile(models.Model):
    ...
    gender = models.BooleanField(_('gender'), help_text=('female is False, mail is True, null is unset.'))
    ...
    def get_nickname(self):
        return self.nick_name if self.nick_name else self.user.useranme
...
class Device(models.Model):
    ...
    device_uuid = models.UUIDField(_('Deice UUID'), null=True)
