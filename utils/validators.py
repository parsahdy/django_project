...
class IDNunmberValidator(RegexValidator):
    regex = '^[0-9]{10}$'
    message = _('Enter a valid id code.')
    code = 'invalid_id_code'


class IBNumberValidator(RegexValidator):
    regex = '^[a-zA-Z]{2}[0-9]{2}[a-zA-Z0-9]{4}[0-9]{7}([a-zA-Z0-9]?){0, 16)$'
    message = _('Enter a valid iban code.')
    code = 'invalid_iban_code'


class BankCardNumbervalidator(RegexValidator):
    regex = '^[0-9]{16}$'
    message = _('Enter a valid card code.')
    code = 'invalid_bank_card_code'
...
validate_id_number = IDNunmberValidator()
validate_iban_number =IBNumberValidator()
